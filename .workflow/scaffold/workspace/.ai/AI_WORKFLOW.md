...
定义：承载业务真源，当前项目的所有上下文，包含但不限于约束（如需求文档、架构文档等）、规范。
...
- **典型内容**：
  ...
  - 契约与定义：API/数据模型/错误模型/事件埋点/领域词典。结构化内容优先以代码承载
  - 架构与决策：业务架构、技术架构、ADR（关键取舍记录，需要固定模版）
  - 质量与验收：DoD/回归点、验收标准与测试清单
- repo-local 的代码/分支/commit 规范、执行 rules、工作流模板等不属于 SoT，属于各 repo 的 `.ai/`
- 此部分会逐步变的巨大，所以后续需要分层，并做严格熵增治理；
- 注意 SoT 与 .ai 文件夹的不同，SoT 为业务真源，.ai 为规则等
...
- 单平台或极简 Demo 的情况下 SoT/EP/Code 可以在同一 repo 中，否则建议 SoT 及不同平台代码多 repo 管理
- [已实现] 多 repo 下，开发期通过 [workspace 聚合](.ai/AI_WORKSPACE.md)
...
- [已实现] 统一的 AI Context Index 定义（协议 + 当前仓库实例），见 [AI_CONTEXT_INDEX.yaml](.ai/AI_CONTEXT_INDEX.yaml)
